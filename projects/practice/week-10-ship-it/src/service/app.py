...
def _to_http_error(exc: Exception) -> tuple[int, str]:
    ...
    raise NotImplementedError("map an exception raised by the RAG pipeline to (status_code, message).")
...
def create_app(rag, *, api_keys: dict[str, str] | None = None,
    ...
    def require_api_key(x_api_key: str | None = Header(default=None)) -> str:
        ...
        raise NotImplementedError("if x_api_key is missing or not in `keys` -> raise HTTPException(401, 'invalid api key').")
    ...
    def ask(req: AskRequest, request: Request, tenant: str = Depends(require_api_key)):
        ...
        raise NotImplementedError("#   trace_id = uuid4().hex[:16]")
