...
class FewShotStrategy(PromptStrategy):
    ...
    def build(self, ticket):
        ...
        import json
        msgs: list[dict] = []
        for subj, body, tier, label in self.examples:
            from .schemas import Ticket as _T, CustomerTier as _C
            ex = _T(id="ex", subject=subj, body=body, customer_tier=_C(tier))
            msgs.append({"role": "user", "content": self._ticket_block(ex)})
            msgs.append({"role": "assistant", "content": json.dumps(label)})
        msgs.append({"role": "user", "content": self._ticket_block(ticket)})
        return BASE_SYSTEM, msgs
...
class ChainOfThoughtStrategy(PromptStrategy):
    ...
    def build(self, ticket):
        ...
        cot = (BASE_SYSTEM + "\n\nBefore answering, think step by step: "
               ...
               "(2) what is the business impact -> which priority? "
               "(3) what is the emotional tone -> which sentiment? "
               "(4) does a human have to take an action (refund, legal, security, account change)? "
               "Then emit the classification.")
        return cot, [{"role": "user", "content": self._ticket_block(ticket)}]
